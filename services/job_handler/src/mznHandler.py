...
def handle_new_mzn_job(data):
    ...
    if k8_result["status"] not in ["ERROR", "FAILED"]:
        dict = {"optVal": data["optVal"], "featureVector": feature_vector, "solverName": solver_name, "executionTime": k8_result["executionTime"], "status": k8_result["status"], "result": k8_result["result"], "instructions": "HandleMznInstance", "queueName": "kbHandler"}
