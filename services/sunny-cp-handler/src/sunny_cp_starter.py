...
def run_solver(solver, problem):
    print("Running solver ", solver.name())
    """
    ...
    if susp:
        print(solver.name(), 'was suspended')
        susp_solver = susp[0]
    ...
    if solver.status == 'ready':
        ...
        print(solver.name(), ' status is mzn2fzn')
        if problem.isCOP() and not solver.obj_var:
            print(solver.name(), ' problem.isCOP is: ', problem.isCOP())
        ...
        print(solver.name(), ' status is flatzinc')
    ...
    if solver.status == 'flatzinc':
        print("solver.name is " + solver.name(), " - ", cmd)
    ...
    print("appending solver: ", solver.name())      
